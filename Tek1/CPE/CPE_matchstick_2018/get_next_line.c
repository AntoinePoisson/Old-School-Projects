...
char *my_strcat(char *dest, char *src)
{
    ...
    if (dest != NULL)
        for (; dest[i] != '\0'; i++)
            result[i] = dest[i];
    for (a = 0; a <= READ_SIZE && src[a] != '\0'; a++, i++)
        result[i] = src[a];
    result[i] = '\0';
    return (result);
}
...
int check_end_line(char *buffer, int return_read)
{
    ...
    if (buffer != NULL) {
        ...
        if (buffer[i] == '\n')
            solv = i;
        else if (return_read == 0 && prev_buffer != buffer) {
            solv = i;
            prev_buffer = buffer;
        }
    }
...
char *rework_buffer(char *buffer, int *read_y, int *nbr_charac)
{
    ...
    if (size > 1 || (size == 1 && buffer[0] == '\n'))
        for (; buffer[i] != '\0'; i++, i_two++)
...
char *algo(int fd, int *line, int read_y, int *nbr)
{
    ...
    *line += 1;
    while ((read_y == 1) && (ret_read = read(fd, reader, READ_SIZE) > 0)) {
        ...
        for (int i = 0; reader[i] != '\0' && i <= READ_SIZE; i++)
            reader[i] = '\0';
...
char *get_next_line(int fd)
{
    ...
    if ((buffer = algo(fd, &line, 1, &nbr_charac)) == NULL ||
