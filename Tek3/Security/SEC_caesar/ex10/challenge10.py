...
def connection(cookie, arg):
    if cookie == "":
        res = requests.post("http://127.0.0.1:5000/challenge10", data=arg)
    else:
        res = requests.post("http://127.0.0.1:5000/challenge10", data=arg, cookies={"x-access-token": cookie})
...
def get_unknown_string(cookie, block_size):
    _, resp = connection(cookie, "")
    unknown_string = 'A' * (len(b64decode(resp)))
    tmp = 'A' * (len(b64decode(resp)) - 1)
    length = len(unknown_string)
    last_size = -1
    ...
    for i in range(len(unknown_string)):
        ...
        for nb in range(255):
            _, resp = connection(cookie, b64encode(bytes(unknown_string + tmp, 'utf-8')).decode())
            resp = b64decode(resp)
            unknown_r = resp[:length]
            tmp_r = resp[length:length * 2]
            last = resp[length * 2:]

            if i == 0 and nb == 0:
                last_size = len(resp)
            if last_size != len(resp) and len(resp) == length * 2:
...
def main(av):
    # cookie, resp = connection("", b64encode(bytes(15 * "A" + "C" + 15 * "A", 'utf-8')).decode())
